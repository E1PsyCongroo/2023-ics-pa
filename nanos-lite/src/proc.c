...
Context* schedule(Context *prev) {
  ...
  if (p == &pcb_boot) {
    DEBUG("switch current(pcb_boot.context(entry: 0x%08x, pdir: %p, GPRx: 0x%08x)) -> pcb[%d].context(entry: 0x%08x, pdir: %p, &argc: 0x%08x)", prev_context_pa->mepc + 4, prev_context_pa->pdir, prev_context_pa->GPRx, current - &pcb[0],  current_context_pa->mepc + 4, current_context_pa->pdir, current_context_pa->GPRx);
  }
  else {
    DEBUG("switch current(pcb[%d].context(entry: 0x%08x, pdir: %p, GPRx: 0x%08x)) -> pcb[%d].context(entry: 0x%08x, pdir: %p, &argc: 0x%08x)", p - &pcb[0], prev_context_pa->mepc + 4, prev_context_pa->pdir, prev_context_pa->GPRx, current - &pcb[0],  current_context_pa->mepc + 4, current_context_pa->pdir, current_context_pa->GPRx);
  }
